( function () {
	...
	/**
	 * Handle a text/event-stream response and forward each JSON event to the client.
	 *
	 * @param {Response} resp   The fetch response object.
	 * @param {?number}  reqId  The request id for error reporting.
	 */
	function handleSseResponse( resp, reqId ) {
				...
				while ( ( index = buffer.indexOf( '\n\n' ) ) !== -1 ) {
					...
					if ( data ) {
						try {
							...
							postError( reqId ?? null, e.message );
						}
					}
				}
			...
			fetch( WPMCPB.streamable_endpoint, {
				...
				.then( ( resp ) => {
					...
					if ( contentType.includes( 'text/event-stream' ) ) {
						handleSseResponse( resp, payload.id );
						return null;
					}
